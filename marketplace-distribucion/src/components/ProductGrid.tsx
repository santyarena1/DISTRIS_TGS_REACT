...
export function ProductGrid({ products, isLoading }: ProductGridProps) {
  ...
  // Helper para formateo de moneda (ej: 1.250,00)
  ...
  const renderPrice = (product: Product) => {
    // 1. PRECIO BASE (NETO EN USD)
    let netPrice = product.price || 0;
    
    // CASO ESPECIAL: TGS y Gaming City traen el PRECIO FINAL (IVA INC).
    // Para hacer los cálculos matemáticos correctamente, primero lo bajamos a NETO.
    ...
    // CÁLCULO DE IIBB (Solo New Bytes: 3% del Neto)
    let iibbAmount = 0;
    if (product.distributorId === 'newbytes') {
      iibbAmount = netPrice * 0.03;
    }

    // --- VISTA 1: DETALLE DE IMPUESTOS (Desglose) ---
    ...
    // --- VISTA 2: MEDIO IVA (Con Reglas Especiales) ---
    if (taxMode === 'half-vat') {
      
      // EXCEPCIÓN A: Gaming City -> Promo 5% OFF sobre el precio FINAL
      if (product.distributorId === 'gamingcity') {
        // Reconstruimos el precio final real
      ...
      // EXCEPCIÓN B: Grupo Núcleo -> Muestra SIN IVA (Neto)
      ...
      // EXCEPCIÓN C: Elit -> Muestra IVA COMPLETO (No aplica medio IVA)
      ...
      // ESTÁNDAR (New Bytes, TGS, etc) -> Medio IVA Matemático
      const halfVatRate = productVatRate / 2;
      const halfVatPrice = netPrice * (1 + (halfVatRate / 100));
      
      // Si es New Bytes, sumamos el IIBB al final
    ...
    // --- VISTA 3: PRECIO FINAL (Default - Neto + IVA + IIBB) ---
    const vatAmount = netPrice * (productVatRate / 100);
    const finalPrice = netPrice + vatAmount + iibbAmount;
